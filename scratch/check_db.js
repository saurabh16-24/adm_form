...
async function check() {
  try {
    const raw = await pool.query('SELECT id, student_name FROM raw_enquiries LIMIT 5');
    console.log('Raw Enquiries:', raw.rows);
    const enq = await pool.query('SELECT id, student_name, raw_id FROM enquiries WHERE raw_id IS NOT NULL ORDER BY id DESC LIMIT 5');
